...
module.exports = {
  ...
  async execute(msg, args, client) {
    if (!msg.guild) {
      await msg.reply("This command only works in a server, not DMs.");
      return;
    }
    ...
    try {
      ...
      if (isReservedName(name) || !isValidStickerName(name)) {
        await msg.reply(`\`${name}\` isn't a valid sticker name or command. Run \`!help st\` for usage.`);
        return;
      }
      ...
      await msg.reply('Something went wrong with that sticker command.').catch(() => {});
    }
  },
};
