...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (sub === 'edit') {
      ...
      if (!parsed) {
        await message.reply('Usage: `!embed edit <message_link>`');
        return;
      }
    ...
    await message.reply('Usage: `!embed create #channel` or `!embed edit <message_link>`');
  },
  ...
  async executeSlash(interaction) {
    ...
    if (sub === 'edit') {
      ...
      if (!parsed) {
        await interaction.reply({ content: 'That does not look like a valid message link.', ephemeral: true });
        return;
      }
      const target = `${parsed.channelId}_${parsed.messageId}`;
      const prompt = buildEditorPrompt('edit', target);
