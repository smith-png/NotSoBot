...
const { brandColor: DEFAULT_COLOR } = require('../config/theme'); // used when no color is provided in the modal
...
async function openEditorModal(interaction) {
  ...
  if (!isStaff(interaction.member)) {
    await interaction.reply({ embeds: [staffEmbed("You don't have permission to use this.")], ephemeral: true });
    return;
  }
    ...
    try {
      ...
      if (msg.author.id !== interaction.client.user.id) {
        await interaction.reply({
          embeds: [errorEmbed('That message was not posted by this bot, so it cannot be edited here.')],
          ephemeral: true,
        });
        return;
      }
      ...
      await interaction.reply({ embeds: [errorEmbed('Could not find that message. Check the link and try again.')], ephemeral: true });
      return;
    }
  }
...
async function handleEditorSubmit(interaction) {
  ...
  if (color === null) {
    await interaction.reply({
      embeds: [errorEmbed('Invalid color format. Use a 6-digit hex code, e.g. 395F3D or #395F3D.')],
      ephemeral: true,
    });
    return;
  }
  ...
  if (!title && !description && !image) {
    await interaction.reply({ embeds: [errorEmbed('Embed needs at least a title, description, or image.')], ephemeral: true });
    return;
  }
  ...
  try {
    ...
    await interaction.reply({
      embeds: [errorEmbed('Something went wrong saving that embed. Check the bot has permission to post/edit in that channel.')],
      ephemeral: true,
    });
  }
}
