...
async function handleAdd(msg, args) {
  if (msg.channel.id !== config.archiveChannelId) {
    await msg.reply(`Post the image and run this in <#${config.archiveChannelId}>.`);
    return;
  }
  ...
  if (!msg.reference) {
    await msg.reply('Reply to the message containing the image you want to save, with `!st add {name}`.');
    return;
  }

  const sourceMessage = await msg.channel.messages.fetch(msg.reference.messageId).catch(() => null);
  if (!sourceMessage) {
    await msg.reply("Couldn't find the message you replied to — it may have been deleted.");
    return;
  }

  const attachment = sourceMessage.attachments.first();
  if (!attachment) {
    await msg.reply('That message has no image attached.');
    return;
  }
  ...
  if (count >= config.maxStickersPerUser) {
    await msg.reply(`You've hit the ${config.maxStickersPerUser}-sticker limit. Delete one first with \`!st delete {name}\`.`);
    return;
  }
  ...
  try {
    ...
    if (e.code === 11000) {
      await msg.reply(`You already have a sticker named \`${name}\`. Pick a different name or \`!st rename\` the old one.`);
      return;
    }
    throw e;
  }
...
async function handleDelete(msg, args) {
  ...
  if (mentioned) {
    if (!isStaff(msg.member)) {
      await msg.reply("You don't have permission to delete another user's sticker.");
      return;
    }
  ...
  if (!name) {
    await msg.reply('Usage: `!st delete {name}`');
    return;
  }
  ...
  if (!deleted) {
    await msg.reply(`No sticker named \`${name}\` found.`);
    return;
  }
...
async function handleRename(msg, args) {
  ...
  if (!oldName || !newName) {
    await msg.reply('Usage: `!st rename {old_name} {new_name}`');
    return;
  }
  ...
  if (!sticker) {
    await msg.reply(`No sticker named \`${oldName}\` found.`);
    return;
  }
  ...
  try {
    ...
    if (e.code === 11000) {
      await msg.reply(`You already have a sticker named \`${newName}\`.`);
      return;
    }
    throw e;
  }
...
async function handleSteal(msg, args) {
  if (!msg.reference) {
    await msg.reply('Reply to a message the bot sent containing a sticker to steal it.');
    return;
  }
  ...
  if (!targetMessage || targetMessage.author.id !== msg.client.user.id) {
    await msg.reply('That message was not a sticker sent by this bot.');
    return;
  }

  const log = await StickerLog.findOne({ messageId: targetMessage.id });
  if (!log) {
    await msg.reply("Couldn't identify a sticker on that message.");
    return;
  }

  const source = await Sticker.findById(log.stickerId);
  if (!source) {
    await msg.reply('The original sticker no longer exists.');
    return;
  }
  ...
  try {
    ...
    if (e.code === 11000) {
      await msg.reply(`You already have a sticker named \`${name}\`. Try \`!st steal {new_name}\` to pick a different one.`);
      return;
    }
    throw e;
  }
...
async function handleList(msg) {
  ...
  const mentioned = msg.mentions.users.first();
  if (!mentioned) {
    await msg.reply('Usage: `!st list @user`');
    return;
  }
...
async function handleSend(msg, name, client) {
  ...
  if (cooldown.onCooldown) {
    await msg.reply(`Slow down — try again in ${cooldown.secondsRemaining}s.`).then((m) => {
  ...
  if (!sticker) {
    await msg.reply(`No sticker named \`${name}\` found. Check \`!st collection\`.`);
    return;
  }
  ...
  if (!attachment) {
    ...
    await msg.reply(`⚠️ The source image for \`${sticker.nameDisplay}\` is missing. Please re-add it with \`!st add\`.`);
    return;
  }
